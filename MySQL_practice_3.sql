...
#第一题
	...
	-- 1.2 增加条件：平均成绩大于70
	select s.NAME as "student name", s.id as "student number", avg(sc.score) as "average score"
...
#第二题
	...
	select s.name as "student name", s.id as "student number", count(sc.course_id), sum(sc.score)
	from student_course sc 
	join student s 
	on sc.student_id = s.id
	group by s.id;
